...
from methods.simplenet import SimpleNetFinetune
...
def feature_evaluation(writer, iter, cl_data_file, model, n_way=5, n_support=5, n_query=15, adaptation=False):
    ...
    if adaptation:
        scores = model.set_forward_adaptation(iter, z_all, writer, is_feature=True)
    else:
        scores = model.set_forward(iter, z_all, writer, is_feature=True)
...
if __name__ == '__main__':
    ...
    params.model, params.ratio, params.loss_type, params.large_margin, time.time())
    ...
    if params.method == 'baseline':
        model = BaselineFinetune(model_dict[params.model], ratio=params.ratio, loss_type=params.loss_type,
                                 margin=params.large_margin, centered=params.centered, **few_shot_params)
    ...
    if not params.method in ['baseline', 'baseline++', 'simplenet']:
    ...
    if params.method in ['maml', 'maml_approx']:  # maml do not support testing with feature
        ...
        writer = SummaryWriter(log_dir='/home/gaojinghan/closer/vis_log/%s' % params.reminder)
        ...
        for i in range(iter_num):
            acc = feature_evaluation(writer, i, cl_data_file, model, n_query=15, adaptation=params.adaptation,
                                     **few_shot_params)
            acc_all.append(acc)
