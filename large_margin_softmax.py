...
class LargeMarginSoftmaxLinear(nn.Module):
    def __init__(self, input_dim, output_dim, margin):
        ...
        self.divisor = math.pi / self.margin # what is this
        self.coeffs = binom(margin, range(0, margin + 1, 2)) # what is this
        self.cos_exps = range(self.margin, -1, -2) # what is this
    ...
    def forward(self, input, target=None):
        if self.training:
            ...
            logit = input.matmul(self.weight) # batch * output
            logit_target = logit[:, target]
            weight_target_norm = self.weight[:, target].norm(p=2, dim=0) # weight normalized in columns
            input_norm = input.norm(p=2, dim=1) # input normalized in rows
